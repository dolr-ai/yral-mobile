package com.yral.shared.analytics.constants

/**
 * All analytic event names used across the app.
 */
enum class FeatureEvents {
    FIRST_APP_LAUNCH,
    SPLASH_SCREEN_VIEWED,
    SPLASH_SCREEN_DURATION,
    APP_ONBOARDING_SHOWN,

    // Auth / Signup / Login
    AUTH_SCREEN_VIEWED,
    SIGNUP_NUDGE_DISMISSED,
    SIGNUP_CLICKED,
    SIGNUP_JOURNEY_SELECTED,
    SIGNUP_INITIATED,
    SIGNUP_SUCCESS,
    LOGIN_SUCCESS,
    AUTH_FAILED,
    ANONYMOUS_AUTH_FAILED,
    AUTH_SESSION_STATE_CHANGED,
    IDENTITY_TRANSITION,
    PHONE_NUMBER_ENTERED,
    OTP_REQUEST_INITIATED,
    OTP_SCREEN_VIEWED,
    OTP_VALIDATION_RESULT,
    OTP_DISMISSED,

    // Home
    HOME_PAGE_VIEWED,
    BOTTOM_NAVIGATION_CLICKED,

    // Video / Feed
    VIDEO_IMPRESSION,
    VIDEO_STARTED,
    VIDEO_VIEWED,
    VIDEO_CLICKED,
    NSFW_ENABLED,
    VIDEO_REPORTED,
    DELETE_VIDEO_INITIATED,
    VIDEO_DELETED,
    VIDEO_DURATION_WATCHED,
    ENABLE_PUSH_NOTIFICATION_POPUP_SHOWN,
    NOTIFICATIONS_ENABLED,
    FEED_TOGGLE_CLICKED,
    FEED_LOADER_SHOWN,
    VIDEO_DOWNLOADED,

    // Game
    GAME_VOTED,
    GAME_PLAYED,
    GAME_TUTORIAL_SHOWN,
    GAME_CONCLUDED_BOTTOMSHEET_CLICKED,
    FORCED_GAMEPLAY_NUDGE_SHOWN,
    HOW_TO_PLAY_CLICKED,

    // Menu
    MENU_PAGE_VIEWED,
    MENU_CLICKED,

    // Upload
    UPLOAD_VIDEO_PAGE_VIEWED,
    VIDEO_CREATION_PAGE_VIEWED,
    SELECT_FILE_CLICKED,
    FILE_SELECTION_SUCCESS,
    VIDEO_UPLOAD_INITIATED,
    VIDEO_UPLOAD_SUCCESS,
    VIDEO_UPLOAD_ERROR_SHOWN,
    VIDEO_UPLOAD_TYPE_SELECTED,
    VIDEO_GENERATION_MODEL_SELECTED,
    CREATE_AI_VIDEO_CLICKED,
    AI_VIDEO_GENERATED,
    AI_VIDEO_REQUEST_SUBMITTED,

    // Profile
    PROFILE_PAGE_VIEWED,
    UPLOAD_VIDEO_CLICKED,
    VIDEO_PUBLISHED,
    EDIT_PROFILE_STARTED,
    EDIT_PROFILE_COMPLETED,
    EDIT_PROFILE_CANCELLED,

    // Wallet
    WALLET_PAGE_VIEWED,
    CENTS_TO_DOLR_CONVERTED,
    THIRD_PARTY_WALLET_TRANSFERRED,
    SATS_TO_BTC_CONVERTED,
    VIDEO_VIEWS_REWARDS_NUDGE_SHOWN,
    HOW_TO_EARN_CLICKED,

    // Referral
    REFERRAL_RECEIVED,
    ATTRIBUTION_FAILED,
    REFER_AND_EARN_PAGE_VIEWED,
    REFERRAL_HISTORY_VIEWED,
    SHARE_INVITES_CLICKED,

    // Leaderboard
    LEADERBOARD_PAGE_VIEWED,
    LEADERBOARD_PAGE_LOADED,
    LEADERBOARD_TAB_CLICKED,
    LEADERBOARD_CALENDAR_CLICKED,
    LEADERBOARD_DAY_SELECTED,

    // Recharge
    AIRDROP_CLAIMED,

    // Share
    VIDEO_SHARE_CLICKED,
    SHARE_APP_OPENED_FROM_LINK,

    // Follow
    USER_FOLLOWED,
    USER_UNFOLLOWED,
    FOLLOWERS_LIST_VIEWED,

    // Create Bot
    CREATE_BOT_CTA_CLICKED,
    BOT_CREATION_STARTED,
    BOT_DESCRIPTION_SUBMITTED,
    BOT_DESCRIPTION_ACCEPTED,
    BOT_PROFILE_GENERATED,
    CREATE_BOT_CLICKED,
    BOT_CREATION_SUCCESS,
    BOT_CREATION_FAILED,
    BOT_ACCOUNT_DELETED,

    // AI Chatbot
    INFLUENCER_CARDS_VIEWED,
    INFLUENCER_CARD_CLICKED,
    CHAT_INFLUENCER_CLICKED,
    CHAT_SESSION_STARTED,
    USER_MESSAGE_SENT,
    AI_MESSAGE_DELIVERED,
    FREE_ACCESS_EXPIRED,
    SUBSCRIPTION_CLICKED,
    SUBSCRIPTION_SUCCESS,
    SUBSCRIPTION_FAILED,

    // Tournament
    TOURNAMENT_SCREEN_VIEWED,
    TOURNAMENT_REGISTRATION_INITIATED,
    TOURNAMENT_REGISTERED,
    TOURNAMENT_STATE_CHANGED,
    TOURNAMENT_JOINED,
    TOURNAMENT_ANSWER_SUBMITTED,
    TOURNAMENT_EXIT_ATTEMPTED,
    TOURNAMENT_EXIT_NUDGE_SHOWN,
    TOURNAMENT_EXIT_CONFIRMED,
    TOURNAMENT_OUT_OF_DIAMONDS_SHOWN,
    TOURNAMENT_ENDED,
    TOURNAMENT_RESULT_SCREEN_VIEWED,
    TOURNAMENT_LEADERBOARD_VIEWED,
    TOURNAMENT_REWARD_EARNED,

    // Subscription
    PRO_NUDGE_IMPRESSION,
    PRO_NUDGE_CLICKED,
    PRO_PLAN_VIEWED,
    PRO_BUY_CLICKED,
    PRO_PAYMENT_RESULT,
    PRO_STATUS_UPDATED,
    CREDITS_CONSUMED,
    CREDITS_EXHAUSTED,

    ;

    fun getEventName(): String = name.lowercase()
}
