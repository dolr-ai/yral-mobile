    ...
    val pickerLauncher =
        rememberLauncherForActivityResult(
            ...
            runCatching {
                ...
                Logger.e(throwable) { "Failed to read selected influencer image" }
            }
        }
        ...
        rememberLauncherForActivityResult(
            ...
            if (success && file != null) {
                runCatching {
                    ...
                    Logger.e(throwable) { "Failed to capture influencer image" }
                }
            }
            if (file != null && file.exists()) {
                if (!file.delete()) {
                    Logger.w { "Unable to delete temp influencer image file" }
                }
            }
        ...
        remember(cameraLauncher, context) {
            {
                val imageFile =
                    runCatching {
                        File.createTempFile("influencer_photo_", ".jpg", context.cacheDir)
                    }.onFailure { error ->
                        Logger.e(error) { "Failed to create temp file for influencer photo" }
                    }.getOrNull()
