...
class TrendingInfluencersPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Influencer> =
        try {
            ...
            val result =
                chatRepository.getTrendingInfluencersPage(
                    limit = limit,
                    offset = offset,
                )
            ...
            Logger.e("TrendingInfluencersPaging", e)
            useCaseFailureListener.onFailure(
                e,
                tag = this::class.simpleName!!,
                message = { "failed to fetch trending influencers" },
