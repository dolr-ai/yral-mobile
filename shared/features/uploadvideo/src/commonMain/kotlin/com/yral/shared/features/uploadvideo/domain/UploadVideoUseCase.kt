...
internal class UploadVideoUseCase(
    ...
    override fun execute(parameters: Params): Flow<Result<UploadState, Throwable>> {
        return repository.uploadVideo(parameters.uploadUrl, parameters.filePath)
            .map { uploadStatus ->
                when (uploadStatus) {
                    is UploadStatus.Error -> Err(uploadStatus.exception)
                    is UploadStatus.InProgress -> Ok(
    ...
    data class Params(val uploadUrl: String, val filePath: String)
}
