...
private val imagePickerLogger = Logger.withTag("AiVideoImagePicker")
...
actual fun rememberAiVideoImagePicker(onImagePicked: (ByteArray) -> Unit): () -> Unit {
    ...
    var coordinator by remember { mutableStateOf<IosAiVideoImagePickerCoordinator?>(null) }

    return remember {
        {
            if (coordinator == null) {
                val newCoordinator =
                    IosAiVideoImagePickerCoordinator(
                ...
                if (isPresented) {
                    ...
                    imagePickerLogger.w { "Unable to present AI video image picker" }
                }
            }
        }
    }
...
private class IosAiVideoImagePickerCoordinator(
    ...
    fun presentPicker(): Boolean {
        ...
        if (rootController == null) {
            imagePickerLogger.e { "Unable to find root view controller for AI video image picker" }
            return false
        }
        ...
        if (!UIImagePickerController.Companion.isSourceTypeAvailable(sourceType)) {
            imagePickerLogger.w { "Photo library is not available for AI video image picker" }
            return false
        }
    ...
    override fun imagePickerController(
        ...
        picker.dismissViewControllerAnimated(true) {
            ...
            if (bytes != null) {
                ...
                imagePickerLogger.e { "Failed to convert selected AI video image to bytes" }
            }
            onDismiss()
        }
    }
